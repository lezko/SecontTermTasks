 ...
 */
public class SimpleBinaryTree<T> implements MyBinaryTree<T> {

    protected class SimpleTreeNode implements MyBinaryTree.MyTreeNode<T> {
        public T value;
    ...
    public int getHeight() {
        class HeightHolder { int height = 0; }
        HeightHolder holder = new HeightHolder();
        BinaryTreeAlgorithms.byLevelVisit(getRoot(), (value, level) -> holder.height = Math.max(holder.height, level));
        return holder.height + 1;
    }
    ...
    private class QueueItem<T> {
        ...
        public QueueItem(SimpleTreeNode node, int level) {
            this.node = node;
            this.level = level;
        }
    }
    ...
    public void setGradient(Color topColor, Color bottomColor) {
        int h = getHeight();
        int dR = (topColor.getRed() - bottomColor.getRed()) / h;
        int dG = (topColor.getGreen() - bottomColor.getGreen()) / h;
        int dB = (topColor.getBlue() - bottomColor.getBlue()) / h;
        ...
        while (!queue.isEmpty()) {
            ...
            int k = item.level + 1;
            item.node.setColor(new Color(topColor.getRed() - k * dR, topColor.getGreen() - k * dG, topColor.getBlue() - k * dB));
        }
    }
}
